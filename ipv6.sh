...
ip -6 route add ${PUBLIC_IPV6::-1}4 via $(docker inspect -f '{{range .NetworkSettings.Networks}}{{.GlobalIPv6Address}}{{end}}' wireguard)

ip -6 neigh add proxy ${PUBLIC_IPV6::-1}2 dev eth0
ip -6 neigh add proxy ${PUBLIC_IPV6::-1}3 dev eth0
ip -6 neigh add proxy ${PUBLIC_IPV6::-1}4 dev eth0
...
cat wireguard_config/peer_raspi/peer_raspi.conf | sed "2s/$/, ${PUBLIC_IPV6::-1}4/" \
    | sed "5s/$/, 2001:4860:4860::8888/" > wireguard_config/peer_raspi/peer_raspi.conf.ipv6
...
echo "<pre>+++++++ Raspi Config +++++++ <pre>" >> /var/www/html/index.html

cat wireguard_config/peer_raspi/peer_raspi.conf.ipv6 >> /var/www/html/index.html
